...
>
> **说明4**：本项目已升级为带侧边栏目录树和交互式代码运行的在线学习站点。内页左侧会展示可展开/收起的树形目录菜单，所有代码块右上角会出现“运行”按钮，支持 Python、JavaScript、HTML/CSS、SQL 等语言在浏览器中直接执行并查看输出。详见 [站点使用说明](#站点使用说明)。

### 站点使用说明

项目根目录下的 `site/` 文件夹是一个静态站点生成器，可以把仓库中的 Markdown 文档构建成带侧边栏目录树和“运行”按钮的 HTML 站点。

**环境要求**：Node.js 16 或更高版本。
...
cd site
npm install
npm run build   # 生成 site/dist/ 目录
npm run serve   # 启动本地服务器，访问 http://localhost:3000
```
...
- **Python**：通过 Pyodide（WebAssembly）在浏览器中运行，首次加载需要下载运行环境（约数秒至十几秒）。
- **JavaScript**：浏览器原生执行。
- **HTML / CSS**：在代码块下方的 iframe 中实时预览渲染效果。
- **SQL**：通过 sql.js 在浏览器中运行 SQLite。
- **Shell / 其他语言**：会提示“暂不支持在浏览器中运行”，但复制功能不受影响。

**说明**：构建时会自动下载代码高亮、Python、SQL 等运行所需依赖到 `site/dist/site/vendor/` 目录，页面加载不依赖外部 CDN，因此可以离线使用、部署到任何静态托管服务（如 GitHub Pages、Vercel、Netlify 等）。Python 运行环境（Pyodide）体积较大，点击“运行”时才按需加载。

**部署**：`site/dist/` 目录中的内容可以部署到任何静态托管服务（如 GitHub Pages、Vercel、Netlify 等）。
